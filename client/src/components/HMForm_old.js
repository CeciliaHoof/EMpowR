    ...
    <Form onSubmit={formik.handleSubmit} style={{ marginTop: "2em" }}>
        ...
        {selectedMetricType.id === 1 && (
          <Form.Field>
            <Input
              ...
              min="0"
              value={systolic}
              onChange={(e) => {setSystolic(e.target.value.toString()); formik.values.content = `${systolic}/${diastolic}`}}
      ...
      <Form.Field>
        ...
        {formik.touched.time_taken && formik.errors.time_taken &&
          (<span style={{ color: "red" }}>Date and time are required. They must be in MM-DD-YYYY HH:MM AM/PM format.</span>)}
      </Form.Field>
      <Form.Field>
        <TextArea
          label="comment"
      ...
      <Button type="button" onClick={() => hideForm(false)}>Cancel</Button>
    </Form>
  );
}
